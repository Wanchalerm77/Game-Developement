...
public class FPModel extends Observable {
	...
	// @ public invariant startingDisc == Disc.PLAYER1 ^ startingDisc ==
	// Disc.PLAYER2;
	...
	/*
	 * @ ensures (\forall int row; 0 < row && row < ROWS; board[row].length ==
	 * COLS)
	 * 
	 * @
	 */
	/*
	 * @ ensures (\forall int i; 0 < i && i < ROWS; (\forall int j; 0 < j && j <
	 * COLS; board[i][j].hasDisc() == false) )
	 * 
	 * @
	 */
	...
	private void setIsWinningLine(int row, int col) {
		isWinningLine = checkLine(new Point(row, 0), new Point(row, COLS - 1)) || // horizontal
	...
	private boolean checkLine(Point start, Point end) {
		...
		/*
		 * make sure we don't end up outside the board by: (0) in all cases;
		 * make sure x < ROWS (x can only be static or increase) (1) if y
		 * increases (dirY > 0); make sure y < COLS (2) if y decreases (dirY <
		 * 0); make sure y >= 0;
		 */
	...
	// @ assignable playerOneWins;
	// @ assignable playerTwoWins;
	// @ ensures (playerOneWins == \old(playerOneWins) + 1) ^ (playerTwoWins ==
	// \old(playerTwoWins) + 1);
